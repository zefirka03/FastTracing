...
bool DDA_blocks(Ray r, inout CollisionInfo cl, int cur_chunk, int cur_side) {
	...
	while (it1 + it < 100) {
		...
		if (((world[cur_chunk * 33353] & (1u << uint(int(curBlock.x) / 16 + int(curBlock.y) / 16 * 2 + int(curBlock.z) / 16 * 4))) == 0) ) {
			...
			curBlock = addCoord(startPoint, r.rayDir * (tm[side] + 0.001));
			startPoint += r.rayDir * (tm[side] + 0.001);
		...
		else if ((world[cur_chunk * 33353 + 1 + uint(int(curBlock.x) / 16 + int(curBlock.y) / 16 * 2 + int(curBlock.z) / 16 * 4)] & (1u << uint(int(curBlock.x) % 16 / 8 + int(curBlock.y) % 16 / 8 * 2 + int(curBlock.z) % 16 / 8 * 4))) == 0) {
			if (it > 0) rLen[side] -= dDist[side];
			else rLen[side] = 0;
			startPoint += rLen[side] * r.rayDir;

			vec3 A = vec3(int(startPoint.x) / 8 * 8, int(startPoint.y) / 8 * 8, int(startPoint.z) / 8 * 8),
				B = A + 8;
		...
		else if ((world[cur_chunk * 33353 + 9 + uint(int(curBlock.x) / 8 + int(curBlock.y) / 8 * 4 + int(curBlock.z) / 8 * 16)] & (1u << uint(int(curBlock.x) % 8 / 4 + int(curBlock.y) % 8 / 4 * 2 + int(curBlock.z) % 8 / 4 * 4))) == 0) {
			if (it > 0) rLen[side] -= dDist[side];
			else rLen[side] = 0;
			startPoint += rLen[side] * r.rayDir;

			vec3 A = vec3(int(startPoint.x) / 4 * 4, int(startPoint.y) / 4 * 4, int(startPoint.z) / 4 * 4),
				B = A + 4;
